...
function getBookingTotal(row) {
  return Number(row.total ?? row.estimate ?? row.estimated_total ?? 0) || 0;
}
...
function buildCustomers() {
  ...
  state.bookings.forEach(row => {
    ...
    if (['paid', 'completed'].includes(row.status)) customer.totalSpend += getBookingTotal(row);
...
function calculateNotifications() {
  const items = [];
  state.bookings.filter(row => ['new_request', 'awaiting_confirmation'].includes(row.status)).forEach(row => items.push({ type: 'booking', icon: 'fa-list-check', title: `New booking from ${getBookingName(row)}`, detail: `${humanDate(row.date)} · ${getServiceName(row.service)}`, time: timestampMs(row.createdAt), tab: 'bookings', id: row.id }));
...
function renderNotifications() {
  ...
  setCount('bookings', state.bookings.filter(row => ['new_request', 'awaiting_confirmation'].includes(row.status)).length);
...
function renderOverview() {
  ...
  const revenue = thisMonth.filter(row => ['paid', 'completed'].includes(row.status)).reduce((sum, row) => sum + getBookingTotal(row), 0);
  const todayBookings = state.bookings.filter(row => row.date === todayKey() && !['cancelled', 'declined'].includes(row.status));
  const kpis = [
    ['New requests', state.bookings.filter(row => ['new_request', 'awaiting_confirmation'].includes(row.status)).length, 'fa-list-check', 'Bookings requiring a decision'],
...
function monthlySeries(months = 6) {
  ...
  for (let index = months - 1; index >= 0; index -= 1) {
    ...
    series.push({ label: start.toLocaleDateString('en-GB', { month: 'short' }), bookings: rows.length, revenue: rows.filter(row => ['paid', 'completed'].includes(row.status)).reduce((sum, row) => sum + getBookingTotal(row), 0) });
  }
  return series;
}
...
function renderConfirmationEditors() {
  const defaults = {
    booking: { eyebrow: 'Request received', title: 'Thank you. Your request is with the team.', message: "FAUSTINA'S SPARKLY SERVICES will check availability and contact you to confirm the final details and payment method.", next: 'We normally respond within one working day.' },
...
const automationDefinitions = [
  ['bookingReceived', 'Booking request received', 'Immediately acknowledge a new booking request.', 'immediate'],
...
function renderReports() {
  ...
  const revenue = rows.filter(row => ['paid', 'completed'].includes(row.status)).reduce((sum, row) => sum + getBookingTotal(row), 0);
