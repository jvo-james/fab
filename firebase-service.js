...
export async function submitBookingRequest(input) {
  const data=validateBookingPayload(input,config);
  const s=await getServices(); if(!s)throw new Error('FIREBASE_NOT_CONFIGURED');
  const {db,firestoreModule:f}=s;
  const lockRef=f.doc(db,'slotLocks',`${data.region}_${data.date}_${data.time_slot}`);
  const bookingRef=f.doc(f.collection(db,'bookingRequests'));
  await f.runTransaction(db,async transaction=>{
    const lock=await transaction.get(lockRef);
    if(!lock.exists()||lock.data()?.status!=='available')throw new Error('SLOT_UNAVAILABLE');
    if(lock.data()?.region!==data.region)throw new Error('REGION_MISMATCH');
    transaction.set(bookingRef,{...data,status:'new_request',createdAt:f.serverTimestamp()});
    transaction.set(lockRef,{...lock.data(),status:'requested',bookingId:bookingRef.id,updatedAt:f.serverTimestamp()});
  });
