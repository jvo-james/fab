  ...
  function openFallbackConfirmation(
    ...
    if (successCopy) {
      successCopy.textContent =
        'Send the request on WhatsApp so the team can confirm the final availability.';
    }
  ...
  form.addEventListener(
    ...
    async (event) => {
      ...
      try {
        ...
        if (!firebaseReady || typeof submitBookingRequest !== 'function') {
          throw new Error('FIREBASE_NOT_CONFIGURED');
        }

        if (!explicitlyPublished) {
          throw new Error('SLOT_UNAVAILABLE');
        }

        // The Firestore transaction locks the admin-published slot first.
        await submitBookingRequest(data);

        // Email both the customer and the admin only after Firebase succeeds.
        let emailDelivered = true;
        try {
          await sendSubmissionEmails({
            type: 'booking',
            templateId: templateFor('booking'),
            data: {
              ...data,
              service_name: estimate.service?.name || data.service,
              time_label: slots.find((item) => item.id === data.time_slot)?.label || data.time_slot,
        ...
        if (statusNode) {
          if (
            ...
            statusNode.textContent =
              'That slot was just taken. Please return to the calendar and choose another time.';
          } else {
            statusNode.textContent =
              'We could not complete the booking online. Please try again or contact the team on WhatsApp.';
          }
